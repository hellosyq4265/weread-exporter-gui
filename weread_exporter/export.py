...
class WeReadExporter(object):
    ...
    async def _load_meta_data(self) -> Dict[str, Any]:
        ...
        if not os.path.isfile(self._meta_path):
            ...
            with open(self._meta_path, "w", encoding="utf-8") as fp:
                json.dump(self._meta_data, fp, ensure_ascii=False)
        else:
            with open(self._meta_path, encoding="utf-8") as fp:
                text = fp.read()
                if text:
    ...
    def copy_images(self, save_dir: str) -> None:
        image_files = [
            name
            for name in os.listdir(self._image_dir)
            if os.path.isfile(os.path.join(self._image_dir, name))
        ]
        if not image_files:
            return
        ...
        for name in image_files:
            shutil.copy2(
                os.path.join(self._image_dir, name),
    ...
    async def export_markdown(
        ...
        if min_interval is None or max_interval is None:
            min_interval = max(5, int(interval * 0.6))
            max_interval = int(interval * 1.4)
        ...
        for index, chapter in enumerate(meta_data["chapters"]):
            ...
            if index < len(meta_data["chapters"]) - 1 and random_interval:
                sleep_seconds = random.uniform(min_interval, max_interval)
                logging.info(
                    "[%s] Sleep %.1fs before next chapter"
                    % (self.__class__.__name__, sleep_seconds)
                )
                await asyncio.sleep(sleep_seconds)
            elif index < len(meta_data["chapters"]) - 1:
                logging.info(
                    "[%s] Random chapter interval disabled"
                    % self.__class__.__name__
                )
