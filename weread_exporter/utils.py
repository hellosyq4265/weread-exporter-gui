...
class ResponseHeaders(dict):
    """Dictionary-compatible response headers with duplicate-value support."""

    def __init__(self, values: Dict[str, str], multi_values: Optional[Dict[str, List[str]]] = None):
        super().__init__(values)
        self._multi_values = {
            key.lower(): list(items)
            for key, items in (multi_values or {}).items()
        }

    def getall(self, key: str, default: Optional[List[str]] = None) -> List[str]:
        values = self._multi_values.get(key.lower())
...
def format_filename(filename: str) -> str:
    for char in ('<', '>', ':', '"', '/', '\\', '|', '?', '*'):
        filename = filename.replace(char, "%%%02x" % ord(char))
    filename = "".join(
        "%%%02x" % ord(char) if ord(char) < 32 else char for char in filename
    )
...
def validate_book_id(book_id: str) -> None:
    if not re.fullmatch(r"[A-Za-z0-9_-]+", book_id):
        raise ValueError("书籍 ID 只能包含字母、数字、下划线和连字符")
